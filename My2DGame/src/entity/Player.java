		...
		if(i != 999) {
			...
			case "Door":
				if(hasKey > 0) {
					gp.playSE(3);
					gp.obj[i] = null;
					gp.ui.showMessage("Who makes a one time use key?");
					hasKey--;
				}
				else {
					gp.ui.showMessage("Hmm... Door's locked.");
				}
				break;
			case "Boots":
				gp.playSE(2);
			...
			case "Chest":
				...
				break;
			case "Revolver":
				gp.ui.showMessage("BANG!");
				break;
			case "WIP":
				gp.ui.showMessage("Hey! Turn around, the game's not finished");
				break;
			}
		}
	}
